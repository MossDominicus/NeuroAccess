...
export default function RegisterForm({ lang }: RegisterFormProps) {
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      try {
        ...
        data = { detail: tf("sendCodeFailed", "Failed to send verification code") };
      }
      if (resp.ok && data.success) {
        ...
        // Dev code is only returned by the backend when DEBUG is on, and we
        // only surface it in development builds (production strips this branch).
        if (process.env.NODE_ENV === "development" && data.dev_code) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setError(tf("passwordMismatch", "Passwords do not match"));
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || tf("registerFailed", "Registration failed"));
      }
    } catch (err: any) {
      setError(err.message || tf("networkErrorMsg", "Network error"));
    } finally {
    ...
    >
      ...
      <div className="w-full max-w-md p-5 sm:p-8 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] mx-3 sm:mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (v === "") {
                  setUsername(v);
                  setError(tf("usernameRequired", "Please enter a username"));
                  return;
                }
                const firstChar = Array.from(v)[0] || "";
                const isLetterStart = /^\p{L}/u.test(firstChar);
                if (!isLetterStart) {
                  setError(tf("usernameMustStartWithLetter", "Username must start with a letter"));
                  return;
                }
                if (/[!@#$%^&*()+\=\[\]{}|\\;:'"`/<>?~.,。]/.test(v)) {
                  setError(tf("usernameNoSpecialChars", "Username cannot contain special characters"));
                  return;
                }
                if (/\s{2,}/.test(v)) {
                  setError(tf("noConsecutiveSpaces", "Username cannot have consecutive spaces"));
                  return;
                }
                ...
                if (vlen >= 1 && vlen <= 20) {
                  ...
                  setError(tf("usernameTooLong", "Username must be at most 20 characters"));
                }
                setUsername(v);
              }}
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5 text-[var(--color-text)]">
              {tf("confirmPassword", "Confirm Password")}
            </label>
            <div className="relative">
              <input
                type={showPassword ? "text" : "password"}
                ...
                className="w-full px-3.5 py-2.5 pr-10 rounded-xl border transition-colors bg-[var(--color-surface)] border-[var(--color-border)] text-[var(--color-text)] placeholder:text-[var(--color-text-secondary)] focus:outline-none focus:border-[var(--color-primary)]/30"
                placeholder={tf("confirmPassword", "Confirm Password")}
                required
              />
              <button
                type="button"
