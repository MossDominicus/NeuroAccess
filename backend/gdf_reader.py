...
class GDF199Reader:
    ...
    # Known field offsets in the GDF 1.99 header (verified against BCI Comp IV 2a)
    # IMPORTANT: These offsets were empirically verified on BCICIV_2a_*.gdf files
    # The GDF 1.99 spec has variable field ordering; these values match actual files.
    OFF_VERSION = 0          # 8 bytes ASCII
    OFF_PATIENT_ID = 8       # 80 bytes ASCII
    OFF_RECORDING_ID = 88    # 80 bytes ASCII
    OFF_SAMPLING_RATE = 0xF8 # int32 LE → verified: 250 for BCI Comp IV 2a
    OFF_NUM_SIGNALS = 0xFC   # int32 LE → verified: 25 for BCI Comp IV 2a (22 EEG + 3 EOG)
    OFF_CHANNEL_LABELS = 0x104  # 16 bytes per label (verified: first label = "Fz")
