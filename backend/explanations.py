...
def _build_prompt(a: Dict, level: str, lang: str) -> str:
    ...
    if level == "student":
        return (
            ...
            f"LENGTH: follow the per-paragraph sentence minimums above (at least 16 sentences total, "
            f"TOTAL at least 500 words). A short answer is a FAILURE — every required sentence must be present and complete.\n\n"
            f"PARAGRAPH 1 — The recording and its quality (write AT LEAST 4 sentences): "
            ...
            f"Each of these four points must be its own full sentence.\n\n"
            f"PARAGRAPH 2 — Brainwave bands in this recording (write AT LEAST 8 sentences): "
            f"Define EACH band — delta, theta, alpha, beta — in its own sentence with its rough frequency range, "
            ...
            f"That is 4 bands × 2 sentences = 8 sentences minimum. Then name which band dominates in THIS recording "
            ...
            f"PARAGRAPH 3 — Noise and artifacts in this recording (write AT LEAST 4 sentences): "
            ...
            f"Each point must be its own full sentence.\n\n"
            f"STYLE RULES:\n"
    ...
    return (
        ...
        f"LENGTH: each paragraph must be at least 5 sentences; TOTAL must be no less than 450 words. "
        f"Under-length output is a FAILURE — expand every paragraph with additional quantified analysis "
        f"(more cross-band ratios, more methodological implications, more concrete preprocessing specifics) until the target is met. "
