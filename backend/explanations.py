...
def _build_prompt(a: Dict, level: str, lang: str) -> str:
    ...
    return (
        ...
        f"TASK: Write a rigorous technical analysis of THIS recording in 3 paragraphs. "
        ...
        f"HARD RULE — NO GENERIC TEXTBOOK FILLER: This explanation must contain ONLY information "
        f"specific to THIS recording. Delete any sentence that would read identically for any EEG file, "
        f"for example: 'sampling rate above Nyquist supports the band range of interest' or "
        f"'duration limits frequency resolution' are generic — only state them when this recording's "
        f"actual values make them a real, quantified limitation (e.g., 'at only 20 s, frequency resolution "
        f"is coarse, ~0.05 Hz'). If a sentence adds no information beyond what the parameters already say, "
        f"omit it.\n\n"
        f"PARAGRAPH 1 — Data overview & methodological notes: "
        f"State channel count, sampling rate, duration, and file size. Then note ONLY the methodological "
