...
def _build_prompt(a: Dict, level: str, lang: str) -> str:
    ...
    return (
        ...
        f"TASK: Write a RIGOROUS, DENSE research analysis of THIS recording in 2 paragraphs. "
        f"Every sentence must carry the highest-value technical information a researcher would act on. "
        f"NO filler, NO restating the parameters, NO generic methodology notes.\n\n"
        f"KILL RULES — delete any sentence that:\n"
        f"  (a) merely restates the parameters (e.g., 'the recording has 21 channels at 500 Hz' is allowed "
        f"ONLY as a brief opening clause, never a full sentence of its own);\n"
        ...
        f"PARAGRAPH 1 — The one most important spectral finding (3-4 sentences): "
        f"Open with the dominant band and its real percentage. Then give the ABSOLUTE average_bandpower "
        f"comparison (μV²/Hz) and QUANTIFY the dominance (e.g., 'alpha is 4.1× theta, 3.2× beta'), "
        f"state whether the profile is physiological (1/f-like) or how far it deviates, and name the "
        f"single finding a researcher would act on. No setup sentences.\n\n"
        f"PARAGRAPH 2 — Quality & what it means for analysis (3-4 sentences): "
        ...
        f"- 2 paragraphs, each 4-5 sentences. TOTAL 250-330 words. Dense, precise, zero filler — every sentence must add "
